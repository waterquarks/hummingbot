...
class WooXExchange(ExchangePyBase):
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(woo_x_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case WooX's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since WooX's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_woo_x_timestamp * 1e3)
            self._last_trades_poll_woo_x_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(woo_x_utils.is_exchange_information_valid, exchange_info["symbols"]):
